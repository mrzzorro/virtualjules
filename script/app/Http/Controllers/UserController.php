...
class UserController extends Controller
{
    ...
    {
    	$user = User::with(['videos', 'photos'])->where('slug',$slug)->first();
    	if ($user) {
    		$videos = $user->videos()->where('status','public')->latest()->paginate(12);
    		$photos = $user->photos()->where('is_approved', true)->latest()->paginate(12);
    		if($request->data)
    		{
                if($videos->isEmpty())
                {
    		...
    		return view('profile',compact('user','videos', 'photos'));
    	}else{
    		return abort(404);
    	}
    }
