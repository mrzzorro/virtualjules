...
<div class="single-comment">
    ...
    <div class="favourite-icon">
        @if(Auth::check())
        <a href="javascript:void(0)" onclick="comment_like('{{ $comment->id }}')"><i id="comment_like{{ $comment->id }}" class="{{ !Auth::User()->favourite_photo_comments->where('pivot.photo_comment_id',$comment->id)->count() == 0 ? 'fas fa-heart' : 'far fa-heart' }}"></i></a>
        @else
...
<div class="single-comment ml-50">
    ...
    <div class="favourite-icon">
         @if(Auth::check())
        <a href="javascript:void(0)" onclick="comment_like('{{ $value->id }}')"><i id="comment_like{{ $value->id }}" class="{{ !Auth::User()->favourite_photo_comments->where('pivot.photo_comment_id',$value->id)->count() == 0 ? 'fas fa-heart' : 'far fa-heart' }}"></i></a>
        @else
            <a href="{{ route('login') }}" class="pjax" onclick="profileshow()"><i id="like" class="far fa-heart"></i></a>
        @endif
    </div>
