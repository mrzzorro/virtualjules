...
<input type="text" id="copy_link" value="{{ route('photo.show',$photo->id) }}" class="offscreen" aria-hidden="true">
<div class="ellipish-list text-center">
  <nav>
    <ul>
      <li>
        @if(Auth::check())
        <a href="javascript:void(0)" class="active" onclick="reportphoto('{{ $photo->id }}')">{{ __('Report Inappropriate') }}</a>
        @else
        <a href="{{ route('login') }}" class="active pjax" onclick="profileshow()">{{ __('Report Inappropriate') }}</a>
        @endif
      </li>
      <li><a class="pjax" href="{{ route('profile.show',$photo->user->slug) }}" onclick="profileshow()">{{ __('View Profile') }}</a></li>
      <li><a target="__blank" href="{{ route('photo.show',$photo->id) }}">{{ __('Go to Photo') }}</a></li>
      <li><a href="javascript:void(0)" onclick="share('{{ $photo->id }}')">{{ __('Share') }}</a></li>
      <li><a href="javascript:void(0)" onclick="copy_link()">{{ __('Copy Link') }}</a></li>
      {{-- <li><a href="javascript:void(0)" onclick="embed('{{ $photo->id }}')">{{ __('Embed') }}</a></li> --}}
